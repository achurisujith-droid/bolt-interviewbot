...
export const VideoInterviewAgent: React.FC<VideoInterviewAgentProps> = ({
  ...
  const [resumeBasedQuestions, setResumeBasedQuestions] = useState<any[]>([]);
  ...
  const loadQuestions = async () => {
    console.log('📄 Loading questions for video interview...');
    try {
      if (resumeAnalysis) {
        console.log('🎯 Generating resume-based questions for:', resumeAnalysis.actualRole || position);
        console.log('📊 Experience:', resumeAnalysis.yearsOfExperience, 'years,', resumeAnalysis.seniority, 'level');
        console.log('🔧 Technologies:', resumeAnalysis.keyTechnologies?.join(', '));
        
        const { generateResumeBasedQuestions } = await import('../utils/resumeAnalyzer');
        const customQuestions = await generateResumeBasedQuestions(resumeAnalysis, position);
        
        if (customQuestions && customQuestions.length > 0) {
          setResumeBasedQuestions(customQuestions);
          console.log('✅ Generated', customQuestions.length, 'resume-based questions');
        } else {
          console.log('⚠️ No custom questions generated, using standard questions');
          setResumeBasedQuestions(getStandardQuestions());
        }
      } else {
        console.log('📝 No resume analysis, using standard questions');
        setResumeBasedQuestions(getStandardQuestions());
      }
    } catch (error) {
  ...
  const getStandardQuestions = () => [
    { text: "Hello! I'm your AI interviewer. Please introduce yourself and tell me about your professional background." },
    { text: "What interests you most about this position and why do you think you'd be a good fit?" },
    { text: "Can you describe a challenging project you've worked on recently and how you overcame obstacles?" },
  ...
  const startMockInterview = () => {
    if (interviewStarted || interviewEnded || !questionsLoaded) {
      console.log('Interview already started or ended, skipping...');
      return;
    }
    
    console.log('🤖 Starting mock interview mode...');
    console.log('📋 Using questions:', resumeBasedQuestions.length, 'questions loaded');
  ...
  const askMockQuestion = async (questionIndex: number) => {
    if (interviewEnded || questionIndex >= resumeBasedQuestions.length) {
      console.log('Interview ended or all questions asked');
    ...
    const currentQuestionText = resumeBasedQuestions[questionIndex]?.text || 'Question not available';
    console.log(`🎤 Asking question ${questionIndex + 1}:`, currentQuestionText);
    
    if (resumeAnalysis && resumeBasedQuestions[questionIndex]?.resumeContext) {
      console.log('📄 Resume context:', resumeBasedQuestions[questionIndex].resumeContext);
    }
    
    setCurrentQuestion(questionIndex);
    setCurrentQuestionText(currentQuestionText);
    setAiSpeaking(true);
    setCandidateResponding(false);
    setWaitingForAnswer(false);
    
    console.log('🎤 Question state set, about to speak question');
    
    // Use browser TTS for mock questions
    try {
      await speakQuestion(currentQuestionText);
    ...
    console.log('🎤 Ready for user to start recording');
  };
  ...
  const handleInterviewComplete = async (data: any) => {
    ...
    if (completionAudio) {
      try {
        ...
        const binaryString = atob(completionAudio);
  ...
  const processResponse = async (audioBlob: Blob, videoBlob?: Blob) => {
    ...
    if (socketRef.current && connectionStatus === 'connected') {
      ...
      setTimeout(() => {
        const questionText = resumeBasedQuestions[currentQuestion]?.text || 'Question not available';
        const resumeContext = resumeBasedQuestions[currentQuestion]?.resumeContext || '';
        ...
        setTimeout(() => {
          if (currentQuestion < resumeBasedQuestions.length - 1) {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="text-sm">
                Question {currentQuestion + 1} of {resumeBasedQuestions.length}
              </div>
            </div>
          </div>
        </div>
          ...
          {connectionStatus === 'disconnected' && !isConnected && (
            <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
              <div className="bg-white rounded-lg p-6 text-center max-w-md">
                ...
                <h2 className="text-lg font-semibold text-gray-800 mb-2">Backend Server Required</h2>
                <p className="text-gray-600 mb-4">
                  The AI video interview requires the backend server to be running.
                  Please start the server with: <code>cd server && npm run dev</code>
                </p>
                <div className="text-sm text-red-600">
                  Server must be running on port 3001 for real AI processing.
                </div>
              </div>
            </div>
          )}
        ...
        <div className="h-1 bg-gray-200">
          <div 
            ...
            style={{ width: `${((currentQuestion + 1) / Math.max(resumeBasedQuestions.length, 1)) * 100}%` }}
          />
        </div>
      </div>
    </div>
